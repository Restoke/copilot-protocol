...
export function isTurnComplete(frame: Frame): boolean {
  const content = frame.content;
  return (
    frame.message_type === "PROTOCOL" &&
    typeof content === "object" &&
    content !== null &&
    (content as Record<string, unknown>).type === "TURN_COMPLETE"
  );
}
...
export function validateTurnLifecycle(frames: Frame[]): string[] {
  const errors: string[] = [];
  ...
  if (turnCompleteIndexes.length !== 1) {
    errors.push(
      `expected exactly 1 TURN_COMPLETE, found ${turnCompleteIndexes.length}`,
    );
    return errors;
  }
